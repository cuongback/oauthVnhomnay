...
module.exports = function(app, options) {
  ...
  function userLogin(req, username, password, done) {
    ...
    models.users.findByUsernameOrEmail(username, function(err, user) {
      ...
      user.hasPassword(password, function(err, matched) {
        if(!matched) {
            return done(null, false, req.flash('loginMessage', 'Your password was incorrect.'));
        }
        if(!user.emailVerified) {
          return done(null, false, req.flash('loginMessage', 'Your account has not been confirmed. Please check your email and confirm your email address.'));
        }
        if (err) {
  ...
  if (options.loginPath !== false) {
     ...
     */
    passport.use('loopback-oauth2-local', new LocalStrategy({passReqToCallback: true}, userLogin));
    ...
    app.post(options.loginPath || '/login', function(req, res) {
        req.assert('username', "You've entered an invalid email address.").isEmail();
        req.assert('username', 'Username is required').notEmpty();
        req.assert('password','Password is require').notEmpty();

        var errors = req.validationErrors(true);
        if(errors) {
          var email = String(_.pluck(_.where(errors, {param: 'username'}), 'msg'));
          var password = String(_.pluck(_.where(errors, {param: 'password'}), 'msg'));
